...
###                                 Finally, 713 subjects remained.                                    ###
##########################################################################################################

library(R.matlab)

#########################################################
### 1. Filter subjects, finally 713 subjects remained ###
#########################################################
...
# nback
#nback_qa <- read.csv("/data/joy/BBL/studies/pnc/n1601_dataFreeze/neuroimaging/nback/n1601_NbackConnectQAData_20170718.csv");
...
# nback QA
df <- df[which(df$nbackExclude==0 & df$nbackExcludeVoxelwise==0), ]
# idemo QA
df <- df[which(df$idemoFcExclude==0 & df$idemoFcExcludeVoxelwise==0), ]
####################################
# After above criteria, 713 subjects remaining.
###################################
...
write.csv(subjid_df, "/data/jux/BBL/projects/pncSingleFuncParcel/Replication/data/pncSingleFuncParcel_n713_SubjectsIDs.csv", row.names=FALSE)
