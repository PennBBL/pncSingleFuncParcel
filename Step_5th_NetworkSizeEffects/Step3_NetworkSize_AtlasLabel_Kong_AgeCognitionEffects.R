...
Label_Folder <- paste0(ReplicationFolder, '/results/SingleParcellation_Kong/WorkingFolder/ind_parcellation_200_30');

SubjectsQuantity = 713;
FeaturesQuantity = 20484;
# Extract Probability
...
{
  print(i)
  Label_File <- paste0(Label_Folder, '/', as.character(BBLID[i]), '.mat');
  Data <- readMat(Label_File);
  AllLabel <- cbind(t(Data$lh.labels), t(Data$rh.labels));
  for (j in 1:17) {
    Cmd_Str = paste0('Data_All_Size[i,', as.character(j), ']<-length(which(AllLabel == ', as.character(j), '));');
