...
# Remove network 4 (limbic), which is noisy
Data_All_Size <- Data_All_Size[, c(1:3, 5:17)];

# Age and cognition effect of network size
Gam_P_Vector_System <- matrix(0, 1, 16);
Gam_Z_Vector_System <- matrix(0, 1, 16);
Gam_P_Cognition_Vector_System <- matrix(0, 1, 16);
Gam_Z_Cognition_Vector_System <- matrix(0, 1, 16);
for (i in 1:16)
{
  print(i);
...
Fig <- ggplot(data, aes(EffectRank, AgeEffects_Z)) +
            ...
            "#7499C2", "#00A131", "#F5BA2E", "#7499C2"), width = 0.8);
...
Fig <- ggplot(data, aes(EffectRank, AgeEffects_Z_Cognition)) +
            ...
            "#E76178", "#F5BA2E", "#7499C2", "#7499C2", "#00A131", 
            "#AF33AD", "#E76178", "#00A131", "#7499C2", "#F5BA2E", 
            "#E443FF", "#00A131", "#F5BA2E", "#F5BA2E"), width = 0.8);
Fig <- Fig + labs(x = "", y = "Cognition Effect (Z)") + theme_classic()
Fig <- Fig + theme(axis.text.x = element_text(size= 17.5, color = "black"), axis.text.y = element_text(size= 24, color = "black"), axis.title=element_text(size = 26))
Fig + theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
             scale_x_discrete(limits = c("Network 6 (Visual)", "Network 3 (DM)",
             "Network 1 (DM)", "Network 14 (FP)", "Network 2 (Motor)", 
             "Network 16 (Motor)", "Network 13 (DA)", "Network 7 (Visual)", 
             "Network 12 (DM)", "Network 15 (DA)", "Network 9 (Motor)", 
             "Network 17 (FP)", "Network 11 (VA)", "Network 8 (DA)", 
             "Network 10 (FP)", "Network 5 (FP)"));
