...
for i = 1:17
    ...
    w_Brain_Age_lh(Index_l) = w_Brain_Age_Matrix(i, 1:length(Index_l));
    V_lh = gifti;
    V_lh.cdata = w_Brain_Age_lh';
    V_lh_File = [VisualizeFolder '/w_Brain_Age_lh_Network_' num2str(i) '.func.gii'];
    save(V_lh, V_lh_File);
    % right hemi
    w_Brain_Age_rh = zeros(1, 10242);
    w_Brain_Age_rh(Index_r) = w_Brain_Age_Matrix(i, length(Index_l) + 1:end);
    V_rh = gifti;
    V_rh.cdata = w_Brain_Age_rh';
    V_rh_File = [VisualizeFolder '/w_Brain_Age_rh_Network_' num2str(i) '.func.gii'];
    save(V_rh, V_rh_File);
    % convert into cifti file
    cmd = ['wb_command -cifti-create-dense-scalar ' VisualizeFolder '/w_Brain_Age_Network_' ...
...
V_lh_File = [VisualizeFolder '/w_Brain_Age_abs_sum_lh.func.gii'];
save(V_lh, V_lh_File);
% right hemi
...
V_rh_File = [VisualizeFolder '/w_Brain_Age_abs_sum_rh.func.gii'];
save(V_rh, V_rh_File);
save([PredictionFolder '/Weight_Age/w_Brain_Age_abs_sum.mat'], 'w_Brain_Age_abs_sum_lh', 'w_Brain_Age_abs_sum_rh', 'w_Brain_Age_abs_sum')
% convert into cifti file
cmd = ['wb_command -cifti-create-dense-scalar ' VisualizeFolder '/w_Brain_Age_abs_sum' ...
